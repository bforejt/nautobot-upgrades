"""Shared IOS-XE install-engine machinery (extracted from iosxe_upgrade.py).

Everything here is PLATFORM-NEUTRAL device mechanics used by more than one
job: the RESTCONF reachability/credential plumbing, the pre-flight gates
(version floor, install mode, free space), the transfer tiers (async xcopy
default with its ledger watch, classic copy fallback), the uuid-keyed
install-engine operations (add/activate/commit/remove) and their ledger
tracking, and the pure parsing helpers those flows share.

Extraction contract (2026-08): every function and method in this module was
MOVED VERBATIM from jobs/iosxe_upgrade.py — zero behavior change. The switch
job inherits `InstallEngineMixin`; the 9800 job composes the same mixin with
its own orchestration. Behavior notes and bench provenance stay in the
docstrings of the moved code; do not fork these helpers per-platform.
"""
...
from .restconf import RestconfError
...
class InstallEngineMixin:
    """Device-mechanics layer shared by the upgrade jobs.

    Host class contract: a Nautobot Job providing `self.logger`,
    `self.create_file` (optional, guarded), and a `self._stop`
    threading.Event created at run() start (cooperative cancel).
    """
