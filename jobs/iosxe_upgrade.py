...
class IOSXEUpgrade(Job):
    ...
    dynamic_groups = MultiObjectVar(
        ...
        description=(
            ...
            "(filter-based groups via their own query against the current "
            "database, never cached membership; static groups via their "
            "assignments). The run log records each group's resolved roster — "
            "run Dry-run first to preview it. The final roster is the union "
            "with any explicitly selected devices; an empty total refuses."
        ),
    )
    ...
    def _resolve_roster(self, devices, dynamic_groups):
        ...
        Dynamic Groups resolve at RUN START against the live database:
        filter-based ('dynamic-filter') and set-based ('dynamic-set') groups
        through their own generate_query() — Nautobot 2.4's .members
        property serves CACHED membership ('If up-to-the-minute accuracy is
        needed…' per its own docstring), and a stale cache selecting the
        wrong fleet is exactly the failure this refuses — while static
        groups use their assignments, which ARE their source of truth.
        Every group's resolved roster is logged (Dry-run is the preview); a
        stored ScheduledJob re-resolves at each fire, so membership drift
        between save and fire is deliberate, auditable behavior. An empty
        group warns loudly (usually a drifted filter); a non-device group
        refuses; an empty TOTAL refuses before anything is touched. There
        is deliberately NO count-confirmation gate: selecting a named group
        is the expressed intention, and the per-device gates plus the
        dry-run-first culture remain the safety net.
        """
        ...
        for group in dynamic_groups or []:
            ...
            if getattr(group, "group_type", "") == DynamicGroupTypeChoices.TYPE_STATIC:
                members = list(group.members)
                how = "static assignments"
            else:
                members = list(Device.objects.filter(group.generate_query()).distinct())
                how = "live filter query"
            if not members:
                self.logger.warning(
        ...
        if not merged:
            raise RuntimeError(
                "No devices to upgrade: nothing selected explicitly and the "
                "selected dynamic group(s) resolved to zero devices. "
                "Refusing the run."
            )
        return merged
    ...
    def _read_partitions(self, client, retries=None, log=None):
        ...
        FIELD-MEASURED (bench 2026-07-30, 9300 17.18.03): this read emits
        ~2 SELinux AVC lines (mount-level statfs touching '/', mnt_t) — NOT
        the ~100-line file-enumeration walk a full listing triggers. It is
        the job's cheapest remaining AVC source and the ONLY native RESTCONF
        source of partition totals. DO NOT "optimize" the fields projection:
        the names-only form (dropping total-size;used-size) bench-FAILED the
        same day — denials AND no response from the device. RFC 8040 depth
        is a post-filter on this backend (device-tested: prunes output,
        still collects/denies) — not an alternative. A release that rejects
        `fields` falls back to the full read — the parsers handle either
        shape. Each caller reads fresh: no cache, no staleness invariants
        (simplification 2026-07-10).
        """
        try:
            return (
                client.get(
    ...
    def _install_add(self, client, image, op_uuid, log, target_fs):
        # The image is ALWAYS on flash by this point (classic copy or xcopy
        # landed it, byte-exact verified). The engine's remote-URL add was
        # bench-validated 2026-07-28 and then REMOVED 2026-07-31: xcopy plus
        # the classic fallback covers every transfer case with one fewer
        # method to bench per train (findings retained in the README).
        ...
        payload = _engine_add_payload(op_uuid, path)
    ...
    def _xcopy_precheck_skip(self, client, image, log, target_fs, partitions_data):
        ...
        if address is not None:
            entries = _find_inventory_by_name(ledger, filename, target_fs)
            for pkg_dir, pkg in entries:
                try:
                    if int(str(pkg.get("pkg-size", "")).strip()) != int(expected):
                        continue  # inventory names a wrong-size twin
                except (ValueError, TypeError):
                    continue
                keyed = _keyed_content_url(address[0], address[1], f"{pkg_dir}/{filename}")
                try:
                    size = _keyed_size_read(client, keyed, filename)
                except RestconfError:
                    size = None
                if size is not None and abs(size - expected) <= C.SIZE_MATCH_TOLERANCE_BYTES:
                    self.logger.info(
                        "Image already on %s byte-exact (%s bytes) — confirmed "
                        "from the engine's package inventory plus a keyed read "
                        "(no filesystem walk); skipping the transfer.",
                        target_fs,
                        size,
                        extra=log,
                    )
            ...
            if not entries:
                # No inventory entry names this file — probe the
                ...
                for cand_dir in _known_flash_dirs(ledger, target_fs):
                    ...
                    try:
                        ...
                        probed = 0  # probe unreliable — the listing decides
                        break
                    probed += 1
                    if size is not None:
                        hit_size = size
                ...
                if hit_size is not None:
                    if abs(hit_size - expected) <= C.SIZE_MATCH_TOLERANCE_BYTES:
                        self.logger.info(
                            "Image already on %s byte-exact (%s bytes) — "
                            "sighted by a keyed read (not in the package "
                            "inventory; e.g. classic-copied); skipping the "
                            "transfer (no filesystem walk).",
                            target_fs,
                            hit_size,
                            extra=log,
                        )
                        return True, hit_size
                    self.logger.warning(
                        "A same-named file already exists on %s with the "
                        "WRONG size (%s bytes, expected %s; keyed read, no "
                        "walk) — this transfer overwrites it.",
                        target_fs,
                        hit_size,
                        expected,
                        extra=log,
                    )
                    return False, hit_size
                if probed:
                    self.logger.info(
                        "Image absent from %s — confirmed walk-free (no "
                        "package-inventory entry; keyed probes of %d "
                        "device-published dir(s) all missed). Proceeding to "
                        "transfer.",
                        target_fs,
                        probed,
                        extra=log,
        ...
        if listing is None:
            self.logger.warning(
                "Could not read the filesystem listing for the transfer "
        ...
        if found:
            self.logger.warning(
                ...
                "(%s bytes, expected %s) — this transfer overwrites it.",
                target_fs,
    ...
    def _xcopy_image(self, client, image, log, target_fs, partitions_data=None, pre_fire_size=None):
        ...
        error_text = _rpc_error_text(response)
        if error_text:
            # Rejected fire = positively terminal (no engine op exists) —
            # eligible for the classic-copy fallback tier.
            raise XcopyFailed(
        ...
        while True:
            ...
            if verdict == "failure":
                ...
                # The engine's own terminal verdict — eligible for fallback.
                raise XcopyFailed(
...
def _engine_add_payload(op_uuid, path):
    """install RPC input for the (always-local) add — uuid + path, nothing else.

    The model's download-timeout leaf is DELIBERATELY NOT SENT (bench,
    2026-07-28, 17.18.03, back when the remote-URL add existed): the device
    interprets the value roughly as SECONDS despite the model saying
    minutes (1-1440) — sending 10 strangled a healthy download, while
    omitting the leaf let the device default (~2000, observed) carry a full
    image. The finding stays here because the same seconds-vs-minutes
    confusion shapes the xcopy timeout leaf handling (sent, x60).
    """
...
def _find_inventory_by_name(data, filename, target_fs):
    """Every (pkg-dir, pkg-data) inventory entry for filename ON target_fs.

    The PRE-transfer form of the inventory lookup: before a fire there is
    no descriptor to supply dest-dir, so entries match on pkg-name plus the
    entry's own ios-dir — the device-published mount<->IOS-filesystem
    mapping (bench-captured 'flash:'), matched with the same shared
    partition rule as every other filesystem decision. Used only WITH a
    keyed corroboration read, never alone, to decide a transfer skip.
    """
    wanted = str(target_fs).rstrip(":").lower()
    found = []
    if isinstance(data, dict):
        if (
            "pkg-dir" in data
            and str(data.get("pkg-name", "")).strip() == filename
            and _partition_matches(data.get("ios-dir"), wanted)
            and isinstance(data.get("pkg-data"), dict)
        ):
            found.append((str(data.get("pkg-dir", "")).rstrip("/"), data["pkg-data"]))
        else:
            for value in data.values():
                found.extend(_find_inventory_by_name(value, filename, target_fs))
    elif isinstance(data, list):
        for item in data:
