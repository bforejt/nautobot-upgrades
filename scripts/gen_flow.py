...
SPINE = [
    ...
    ("d_floor", "dec", "Version parses\n& ≥ 17.3.1?",
     {"abort": ("No", "Unknown version, or below the\n17.3.1 RESTCONF-install floor"),
      "passlabel": "Yes"}),
    ("mode", "proc", "Read install-oper boot mode\n(all members)", {}),
    ("d_mode", "dec", "All members\nINSTALL mode?",
     {"abort": ("bundle / bad",
                "BUNDLE, unrecognized mode, or\ninstall-oper unreadable (fail-closed)"),
      "warn": ("bundle-derived /\nopt-in",
               "install-bundle, or absent mode-leaf +\nassume_install_mode → warn, proceed"),
      "passlabel": "Yes"}),
    ...
    ("d_dry", "dec", "Dry-run?",
     {"okright": ("Yes", "DONE: DRY-RUN — pre-flight\npassed, no changes made"),
      "passlabel": "No"}),
    ("copy", "proc", "Copy image to flash:\n(device-initiated copy RPC)", {}),
    ("d_size", "dec", "On-device size\nvs expected?",
     {"abort": ("mismatch", "Size mismatch — incomplete\nor corrupt transfer"),
      "warn": ("unknown", "Size unknown/unreadable\n→ warn, rely on signature"),
      "passlabel": "match"}),
    ("add", "proc", "install add (poll target version\n→ staged; warn if unconfirmed)", {}),
    ("act", "proc", "install activate → reload\n(arm auto-abort timer)", {}),
    ...
    ("d_confirm", "dec", "Target version\nstably confirmed?",
     {"abort": ("No", "Not confirmed before timeout — NOT\ncommitted; auto-rollback should revert"),
      "passlabel": "Yes"}),
    ("rollbackchk", "proc", "Check auto-rollback timer armed\n(warn if not confirmable)", {}),
    ("commit", "proc", "install commit → verify\n(scoped to target version)", {}),
    ("d_commit", "dec", "Commit\nsucceeded?",
